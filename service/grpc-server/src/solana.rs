    ...
    T: serde::de::DeserializeOwned,
{
    let wire_output = match encoding {
        TransactionBinaryEncoding::Base58 => {
            if encoded.len() > MAX_BASE58_SIZE {
                metrics::highway::increment_transaction_decode_error("base58_size_exceeded");
            ...
            match bs58::decode(encoded).into_vec() {
                Ok(v) => v,
                Err(e) => {
                    metrics::highway::increment_transaction_decode_error("invalid_base58");
        ...
        TransactionBinaryEncoding::Base64 => {
            if encoded.len() > MAX_BASE64_SIZE {
                metrics::highway::increment_transaction_decode_error("base64_size_exceeded");
                return Err(invalid_params(format!(
            ...
            match BASE64_STANDARD.decode(encoded) {
                Ok(v) => v,
                Err(e) => {
                    metrics::highway::increment_transaction_decode_error("invalid_base64");
    ...
    if wire_output.len() > PACKET_DATA_SIZE {
        metrics::highway::increment_transaction_decode_error("decoded_size_exceeded");
        return Err(invalid_params(format!(
        ...
        .deserialize_from(&wire_output[..])
    {
        ...
        Err(err) => {
            metrics::highway::increment_transaction_deserialize_error("bincode");
